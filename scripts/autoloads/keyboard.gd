...
func _ready() -> void:
	if InputMap.has_action("player-{n}_*".format({"n":0})) == false:
		...
		var tool_mouse_action : String = "player-{n}_tool_mouse".format({"n":0})
		...
		var tool_key_action : String = "player-{n}_tool_key".format({"n":0})

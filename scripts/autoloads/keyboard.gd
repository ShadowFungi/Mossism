...
#func load_new_map():
	...
	#InputMap.action_add_event(aim_action, aim_action_event)
	## Input used to make player 1 grab ledges
	#var ledge_grab_action : String = "player-{n}_ledge_grab".format({"n":1})
	#var ledge_grab_action_event : InputEventKey
	#ledge_grab_action_event = InputEventKey.new()
	#ledge_grab_action_event.keycode = InputRemapper.ledge_grab_key
	#InputMap.action_erase_events(ledge_grab_action)
	#InputMap.action_add_event(ledge_grab_action, ledge_grab_action_event)
